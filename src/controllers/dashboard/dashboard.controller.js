...
const endOfDay = new Date(Date.UTC(yesterday.getUTCFullYear(), yesterday.getUTCMonth(), yesterday.getUTCDate(), 23, 59, 59));
...
const ingresosDia = await Booking.aggregate([
  { $match: { date: { $gte: startOfDay, $lte: endOfDay }, status: "confirmed" } },
  {
    $lookup: {
